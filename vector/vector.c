...
vector *vector_create(copy_constructor_type copy_constructor,
    ...
    vect->copy_constructor = copy_constructor;
    vect->destructor = destructor;
    vect->default_constructor = default_constructor;
    vect->array = calloc(INITIAL_CAPACITY, sizeof(void *));
    vect->size = 0;
...
void vector_resize(vector *this, size_t n) {
    ...
    assert(n >= 0);
    // your code here
    size_t actSize = vector_size(this);
	size_t actCap = vector_capacity(this);
	if(n != actSize){
		if(n > actSize){
			if(n > actCap){
...
void vector_push_back(vector *this, void *element) {
    ...
    this->size++;
    this->array[this->size] = this->copy_constructor(NULL);
}
